...
def _apply_hide_rules_to_summary(summary_data):
    ...
    income/expense/net totals. Also removes now-empty categories so you don't see
    e.g. 'Misc $0' after pruning.
    """
    EPS = 0.005
    ...
    def _is_robinhood_450(t) -> bool:
        # Hide ONLY the Robinhood debit of -$450.00
        a = _amt(t)
        d = _desc(t)
    ...
    def _prune_and_total(node):
        """
        Recursively:
          - drop hidden transactions at leaves
          - compute totals bottom-up
        Returns the node's signed total.
        """
    ...
    def _prune_empty_nodes(node):
        """
        Remove empty children (no transactions, no children, ~zero total).
        Returns True if the node should be kept; False if it should be dropped.
        """
        if not isinstance(node, dict):
            return False
        ...
        # If it has children left, keep it
        if new_children:
            return True

        # Leaf: keep only if it still has visible transactions or a non-zero total
    ...
    for _mk, month_blob in summary_data.items():
        ...
        # 1) Prune txs & recompute totals bottom-up
        tree = month_blob.get("tree") or []
        for top in tree:
            _prune_and_total(top)

        # 2) Drop empty categories produced by pruning
        pruned_tree = []
        for top in tree:
        ...
        # 3) Recompute per-month rollups by scanning the (now-pruned) leaves
        income_sum = 0.0
        expense_sum = 0.0
...
# ======== BEGIN: Walter patches for rolled-up/pruned totals ========
# Ensure every page/API uses the same pruned summary and rebuilt categories so Income/JL Pay appears everywhere.

# Safe imports (idempotent)
try:
...
# ======== BEGIN: consolidated helpers (pruning + categories rebuild) ========
...
def _rebuild_categories_from_tree(summary_data: dict) -> None:
    ...
    read categories (not the tree) include Income/JL Pay etc.
    """
    if not summary_data:
        return
...
def api_recent_activity():
    ...
    # de-noise and sort by biggest absolute change; take top 10
...
def api_category_movers():
    ...
    Reuses /api/recent-activity and returns rows in the shape the card expects.
    """
    ra_resp = api_recent_activity()  # reuse the existing function (returns a Flask Response)
    try:
        data = ra_resp.get_json() or {}
    except Exception:
        # Fallback: if get_json fails for any reason, return empty result
...
def api_categories_monthly():
    ...
    Default: top-level only (compatible with your old charts).
    deep=1 : walk the full summary tree and emit deep paths like:
             "Income / Rachel Pay / Paralon", "Expenses / Vehicles / Fuel", etc.
    """
    ...
    if not deep:
        ...
        categories = [{"name": cat, "path": [cat], "monthly": series}
                      for cat, series in top_bucket.items()]
        categories.sort(key=lambda c: sum(c["monthly"]), reverse=True)
        return jsonify({"months": months, "categories": categories})
    ...
    def walk(node: dict, path_parts: list[str], month_index: int):
        ...
        key = " / ".join(path_now)  # <- charts split on " / " to rebuild the tree
    ...
    categories = [{"name": key, "path": key.split(" / "), "monthly": series}
                  for key, series in deep_bucket.items()]
    categories.sort(key=lambda c: sum(c["monthly"]), reverse=True)
        ...
        for sub, ssubdict in (subdict or {}).items():
            for ssub, sssdict in (ssubdict or {}).items():
                for sss in (sssdict or {}).keys():
                    s = str(sss).strip()
                    if s: seen_sss[s].add((cat, sub, ssub))
...
def api_subscriptions():
    ...
    def _parse(dt):
        return _parse_any_date(dt) if dt else None
    ...
    def norm_merchant(desc: str) -> str:
        if not desc:
            return "(unknown)"
        s = str(desc).upper()
        # normalize broadly: remove punctuation (including apostrophes) and digits → spaces
        for ch in "0123456789'\"*#-_.\\/(),[]:;@!&+$%^~`?{}<>=|":
            s = s.replace(ch, " ")
        noise = ("ONLINE", "PURCHASE", "PAYMENT", "AUTOPAY", "SUBSCRIPTION",
                 "RECURRING", "WWW", "COM", "INC", "LLC", "CORP", "THE")
        for w in noise:
            s = s.replace(f" {w} ", " ")
    ...
    # For compatibility, echo back the effective window string
...
def api_recurrents():
    """
    Recurring streams (config-driven) + Variable income estimate (mobile deposits & tips).
    """
    import importlib
    ...
    def _cmp(s: str) -> str:
        s = (s or "").upper()
        # strip all non-alnum for robust substring checks
    ...
    def norm_merchant(desc: str) -> str:
        ...
        for ch in "0123456789'\"*#-_.\\/(),[]:;@!&+$%^~`?{}<>=|":
            s = s.replace(ch, " ")
        for w in ("ONLINE","PURCHASE","PAYMENT","AUTOPAY","SUBSCRIPTION",
                  "RECURRING","WWW","COM","INC","LLC","CORP","THE"):
            s = s.replace(f" {w} ", " ")
        return " ".join(s.split()).strip() or "(unknown)"

    # Optional canonical alias map
    ...
    def allow_tx(cat_top: str, subcat: str, raw_desc: str, amt: float) -> bool:
        ...
        if (("PAYMENTUS" in desc_up and "SARASOTA" in desc_up) or
            ("SARASOTA" in desc_up and "UTILIT" in desc_up)):
            return True
    ...
    if not flat and not VINC_ENABLED:
        return jsonify({
            ...
            "streams": [], "upcoming": [], "by_week": [], "by_month": [],
            "transactions": [],
            "floor": 0.0, "floor_by_category": [],
            "income_expected": 0.0, "income_recurring": 0.0,
            "variable_income_monthly": 0.0, "variable_income_weekly": 0.0,
            "variable_income_weeks_used": 0,
            "leftover": 0.0, "this_week_due": 0.0,
            "top_fixed_bills": [], "top_fixed_merchants": [],
            "by_week_net": [], "projected_month": {},
    ...
    def biweekly_cap_for(merchant_key: str, rows_subset) -> int:
        ...
        if cmpk in BI_CAP_CMP:
            return BI_CAP_CMP[cmpk]
        if looks_like_income(rows_subset, merchant_key):
            return 3
    ...
    def is_income_stream(s):
        return bool(s.get("is_income"))
    ...
    floor_by_category = [
        ...
        for k, v in sorted(floor_by_cat_map.items(), key=lambda kv: kv[1], reverse=True)
    ]
    ...
    if VINC_ENABLED:
        ...
        def consider_income(date_obj, amount, desc_up="", subcat_up="", is_manual=False):
            if not date_obj or date_obj < win_cut:
                return
            if amount <= 0:
                return
            ...
            if not hit:
                return
            if any(x in desc_up for x in VINC_EXC_MERCH):
                return
            if any(k in desc_up for k in RC_INCOME_KEYS):
                return
    ...
    for ev in upcoming:
        ...
        if not d:
            continue
        if d < start_monday or d > end_date:
            continue
...
def api_forecast():
    ...
    # Use last ~3 months of net cash flow to estimate weekly rates
    weekly_samples = []
    for mk in recent:
...
def api_tx_all():
    ...
    # Live config + unified monthly summary
    monthly, cfg_live = build_monthly()
    months_sorted = sorted(monthly.keys(), key=_norm_month)
    if not months_sorted:
    ...
    # de-dupe signature (covers cats vs tree double-inclusion)
    def _sig(t, cat_hint=""):
...
def all_transactions_page():
    """Render the interactive All Transactions explorer."""
...
# ======== BEGIN: Walter endpoint: /admin/debug/income_probe ========
