...
logging.basicConfig(filename=LOGS_DIR / "plaid_fetch.log", level=logging.INFO,
...
def _die(msg: str, code: int = 1):
    print(f"❌ {msg}", file=sys.stderr); logging.error(msg); sys.exit(code)

if not all([PLAID_CLIENT_ID, PLAID_SECRET, PLAID_ENV]): _die("Missing PLAID_* env vars.")
if PLAID_ENV not in PLAID_ENV_HOSTS: _die(f"Invalid PLAID_ENV: {PLAID_ENV}")

configuration = Configuration(host=PLAID_ENV_HOSTS[PLAID_ENV],
...
def load_access_token(noninteractive: bool | None = None) -> str:
    ...
    if noninteractive:
        raise RuntimeError("Missing access token. Set PLAID_ACCESS_TOKEN or put it in /var/data/config/plaid_access_token.json")

    public_token = input("Enter public_token from Plaid Link: ").strip()
    if not public_token: raise RuntimeError("No public_token provided.")
...
def _to_tx_list(obj):
    if isinstance(obj, dict): arr = obj.get("transactions") or obj.get("items") or []
...
def main():
    ...
    p.add_argument("--since"); p.add_argument("--end")
    ...
    logging.info(f"Fetching {start_date}..{end_date}")
    ...
    all_fetched, offset, count = [], 0, 100
    try:
        while True:
            opts = TransactionsGetRequestOptions(count=count, offset=offset)
            req  = TransactionsGetRequest(access_token=access_token, start_date=start_date, end_date=end_date, options=opts)
            ...
            print(f"📦 Fetched {len(batch)} (Total: {len(all_fetched)}/{resp.total_transactions})")
            if len(all_fetched) >= resp.total_transactions: break
            offset += len(batch)
    except ApiException as e:
        _die(f"API Error: {e}")

    ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    dump_path = OUT_DIR / f"plaid_{ts}.json"
    ...
    logging.info(f"Saved dump to {dump_path}")
    ...
    logging.info(f"Master updated with {len(new_txns)} new cleared")

if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "link":
        print("✅ Your new link_token:\n" + create_link_token())
    else:
        main()
